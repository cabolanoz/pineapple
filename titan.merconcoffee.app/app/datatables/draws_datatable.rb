  ...
  def as_json(options = {})
    {
      ...
      iTotalRecords: OpsBuildDraw.where("build_draw_ind = ?", 1).where("delivery_active_status_ind = ?", 1).count,
      iTotalDisplayRecords: draws.total_entries,
      aaData: data,
